...
import { Edit2, Trash2, Download, Calendar, Share } from 'lucide-react';
    ...
    <div className="space-y-6 mb-24 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#1e293b] p-6 rounded-2xl shadow-lg border border-gray-700">
        <div>
          ...
          <p className="text-sm text-gray-400">Descargue el consolidado para cargar a SharePoint o análisis offline.</p>
        </div>
        <button 
      ...
      <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-xl flex items-start gap-3">
        ...
        <div className="text-xs text-gray-300">
            ...
            <p>Use la pestaña <code className="text-blue-200">Plan_de_Accion</code> para alimentar listas de seguimiento automático en su entorno corporativo.</p>
        </div>
      </div>
      ...
      <div className="bg-[#1e293b] rounded-2xl shadow-lg border border-gray-700 overflow-hidden">
        <div className="p-4 bg-[#0f172a] border-b border-gray-800 flex items-center gap-2">
            ...
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Historial de Auditorías</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-800">
            ...
            <tbody className="divide-y divide-gray-800">
              {records.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-gray-500">No hay auditorías registradas todavía.</td>
                </tr>
              ) : (
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      ...
                      <button 
                        type="button"
                        onClick={() => {
                          if (window.confirm('¿Desea eliminar este registro y TODOS sus hallazgos asociados permanentemente?')) {
                            onDelete(r.id);
                          }
                        }} 
